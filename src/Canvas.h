...
public:
  ...
  // Flat 2-D arrow: shaft from (x0,y0) to (x1,y1) with a filled head at (x1,y1).
  ...
  void arcText(const char* s, int cx, int cy, float radius, float centerDeg,
    ...
    // Standalone sprite - no parent pointer so LovyanGFX doesn't register
    // it as a child of the framebuffer, which can produce arc-shaped artifacts
    // via parent-child lifecycle calls during createSprite/deleteSprite.
    LGFX_Sprite glyph;
